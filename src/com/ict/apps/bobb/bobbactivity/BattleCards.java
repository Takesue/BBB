...
//public class BattleCards extends View {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			...
			// 詳細画面表示を消す
			((BattleActivity)this.activity).invisibleCardDetail();
...
//		this.mHandler.removeCallbacks(this.mUpdateTimeTask);
	...
	public void startDeleteCard(BattleLayout vgroup){
		this.moveView = this;
		this.vgroup = vgroup;
	...
	public void flippedCardFace() {
		...
		if (type == 4) {
			...
			// 表示文字設定
			((TextView)this.findViewById(R.id.myCardValue)).setText(this.cardInfo.getAttack());
		}
		if (type == 5) {
			// 攻、守
			((TextView)this.findViewById(R.id.myCardAD)).setText("守備：");
			// 表示文字設定
			((TextView)this.findViewById(R.id.myCardValue)).setText(this.cardInfo.getDefense());
		}
